/**
 * Скрипт для автоматического перезапуска UniFarm при сбоях
 * 
 * Этот скрипт запускает UniFarm и автоматически перезапускает его при сбоях
 * с функциями мониторинга и защитой от цикличных перезапусков
 */
  ...
  appProcess.on('close', async (code) => {
    ...
    if (code === 0) {
      ...
      if (checkRestartLimit()) {
        ...
        await logEvent(`Для ручного перезапуска выполните команду: node auto-restart.mjs`);
      }
    }
  });
